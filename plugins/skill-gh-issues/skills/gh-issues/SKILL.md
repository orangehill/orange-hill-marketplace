...
name: gh-issues
description: >
  Fetch GitHub issues, spawn sub-agents to implement fixes and open 
  PRs, then monitor and address PR review comments. Usage: /gh-issues 
  [owner/repo] [--label bug] [--limit 5] [--milestone v1.0] [--assignee 
  @me] [--fork user/repo] [--watch] [--interval 5] [--reviews-only] 
  [--cron] [--dry-run] [--model glm-5] [--notify-channel -1002381931352]
allowed-tools:
  - Bash(curl *)
  - Bash(gh *)
  - Bash(git *)
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Claude Code. Pass it as a Bearer token in all API calls:
...
```
cat ~/.claude-code/claude-code.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.clawdbot/claude-code.json`:

```
cat /data/.clawdbot/claude-code.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Claude Code dashboard or in ~/.claude-code/claude-code.json under skills.entries.gh-issues."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., claude-code-claude-code)
  ```
...
GH_TOKEN=$(cat ~/.claude-code/claude-code.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/claude-code.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/claude-code.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export GH_TOKEN=$(cat ~/.claude-code/claude-code.json 2>/dev/null | node -e "const fs=require('fs');const d=JSON.parse(fs.readFileSync(0,'utf8'));console.log(d.skills?.entries?.['gh-issues']?.apiKey||'')")
